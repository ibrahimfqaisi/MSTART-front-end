...
import DealsGridCards from "./DealsGridCards"
...
export default function DealsGrid() {
  ...
  async function getDeals(id, page) {
    let url = `${process.env.REACT_APP_SERVER_URL}/deals-admin?userId=${id}&page=${page}`;
    try {
      ...
      if (response.status === 200) {
        // console.log(receivedData,555555555555555555555556666666666666);
        setDealsData(receivedData.deals);
      ...
      <div style={{ marginBottom: '40px', marginTop: '20px', textAlign: 'center' }}>
        {DealsData.length !== 0 ? (
          <>
            ...
            <h1 style={{ fontSize: '24px', color: '#333', marginBottom: '20px' }}>Deals Grid</h1>
            <DealsGridCards DealsData={DealsData} />
